...
import { GraduationCap, Search, Download, Eye, BookOpen, DollarSign } from "lucide-react";
...
const AdminLearners = () => {
  ...
  const getLearnerStats = (userId: string) => {
    ...
    return { enrolled: learnerEnrollments.length, totalSpent, completed };
  };
  ...
  const exportCSV = () => {
    const headers = ["Name", "Country", "City", "Industry", "Job Title", "Contact", "WhatsApp", "LinkedIn", "Courses Enrolled", "Total Spent", "Joined"];
    const rows = filtered.map((l) => {
      const s = getLearnerStats(l.user_id);
      return [l.full_name, l.country, l.city, l.industry, l.job_title, l.contact_number, l.whatsapp_number, l.linkedin_profile, s.enrolled, `$${s.totalSpent.toFixed(2)}`, new Date(l.created_at).toLocaleDateString()];
    });
  ...
  if (selectedLearner) {
    ...
    return (
      <div className="space-y-6">
        <button onClick={() => setSelectedLearner(null)} className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground">
          ← Back to Learners
        </button>
        <div className="flex items-center gap-4">
          <div className="flex h-14 w-14 items-center justify-center rounded-full bg-blue-500/20 text-blue-400 text-xl font-bold">
            {selectedLearner.full_name?.[0] || "L"}
          </div>
          <div>
        ...
        <div className="rounded-xl border border-border bg-card p-5">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {[
              ["Phone", selectedLearner.contact_number],
              ["WhatsApp", selectedLearner.whatsapp_number],
              ["LinkedIn", selectedLearner.linkedin_profile],
              ["City", selectedLearner.city],
              ["Country", selectedLearner.country],
              ["Industry", selectedLearner.industry],
              ["Job Title", selectedLearner.job_title],
              ["Experience", selectedLearner.experience_level],
              ["Education", selectedLearner.education],
            ].map(([label, val]) => (
              <div key={label}>
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <div className="rounded-xl border border-border bg-card p-4">
            ...
            <p className="text-2xl font-bold text-foreground">{s.enrolled}</p>
            <p className="text-xs text-muted-foreground">Courses Enrolled</p>
          </div>
          <div className="rounded-xl border border-border bg-card p-4">
            <GraduationCap className="h-5 w-5 text-green-400 mb-2" />
            <p className="text-2xl font-bold text-foreground">{s.completed}</p>
            <p className="text-xs text-muted-foreground">Completed</p>
          </div>
          <div className="rounded-xl border border-border bg-card p-4">
            <DollarSign className="h-5 w-5 text-blue-400 mb-2" />
            <p className="text-2xl font-bold text-foreground">${s.totalSpent.toFixed(2)}</p>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <div className="h-1.5 w-20 rounded-full bg-secondary">
                              <div className="h-1.5 rounded-full bg-primary" style={{ width: `${e.progress_percent || 0}%` }} />
                            </div>
              ...
              {filtered.map((l) => {
                ...
                return (
                  <TableRow key={l.id}>
                    ...
                    <TableCell>
                      <button onClick={() => setSelectedLearner(l)} className="text-primary hover:text-primary/80">

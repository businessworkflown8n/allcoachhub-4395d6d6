...
import {
  Plug, CheckCircle2, XCircle, AlertTriangle, RefreshCw, Settings,
...
const AdminIntegrationsHub = () => {
  ...
  const handleConnect = (platformId: string) => {
    toast.info(`Integration with ${platformId} requires API credentials. This feature will be available once API keys are configured.`);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <Card><CardContent className="p-4 text-center"><p className="text-2xl font-bold text-red-400">{errorCount}</p><p className="text-xs text-muted-foreground">Errors</p></CardContent></Card>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex gap-1">
                          {conn?.status === "connected" ? (
                            ...
                            <Button size="sm" variant="outline" onClick={() => handleConnect(item.id)}>Connect</Button>
                          )}
                        </div>
                      </div>
                      {conn?.last_sync_at && (
